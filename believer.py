...
def update_belief_state(g,b,hit,miss):
...
def rollout(g, b, hit, miss, gamma, Rhit, Rmiss):
    rows = g.row_size
    cols = g.col_size

    b_up = update_belief_state(g, b, hit, miss)
    print('this is bup in the loop',b_up)
            ...
            if [r, c] in hit or [r, c] in miss:
                ...
                for r2 in range(rows):
                    for c2 in range(cols):
                        ...
                        sum_trans_util += trans_hit * U_next_is_hit + trans_miss * U_next_is_miss
                currQ = R + gamma * sum_trans_util
            Q[r,c] = currQ
